...
import HighDimensionalProbability.Appendix.Infra.SLT.GaussianLSI.Entropy
import HighDimensionalProbability.Appendix.Infra.SLT.GaussianLSI.TwoPoint
import HighDimensionalProbability.Appendix.Infra.SLT.GaussianPoincare.Limit
...
/-- Each coordinate gradient term is invariant when its base point is flipped:
    the two endpoint values are exchanged, while their squared difference is unchanged. -/
theorem gradient_term_symmetric {n : ℕ} (j : Fin n) (h : (Fin n → Bool) → ℝ)
    (ε : Fin n → Bool) :
    (h (flipCoord j ε) - h (flipCoord j (flipCoord j ε))) ^ 2 =
      (h ε - h (flipCoord j ε)) ^ 2 := by
  have hinv : flipCoord j (flipCoord j ε) = ε := by
    simpa only [Function.comp_apply, id_eq] using congrFun (flipCoord_involutive j) ε
  rw [hinv]
  ring
