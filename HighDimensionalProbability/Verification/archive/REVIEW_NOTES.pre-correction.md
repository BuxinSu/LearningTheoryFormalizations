# Whole-book review notes and formalization hand-off
...
The in-scope census has **873** conclusions: **717 proved**, **55 partial** (45 original `PARTIAL` rows plus 10 conservatively published `UNSURE` rows), **34 missing**, and **67 in appendix**. No current core row is classified `open`. The **191** wholly non-load-bearing practice exercise numbers are intentionally outside this census.

| Chapter | Proved | Partial | Missing | In appendix | Total | Practice exercise numbers out of scope |
|---|---:|---:|---:|---:|---:|---:|
| Appetizer | 2 | 1 | 8 | 0 | 11 | 6 |
| Chapter 1 | 60 | 0 | 2 | 1 | 63 | 6 |
| Chapter 2 | 66 | 10 | 2 | 1 | 79 | 29 |
| Chapter 3 | 80 | 24 | 7 | 0 | 111 | 31 |
| Chapter 4 | 113 | 6 | 1 | 0 | 120 | 23 |
| Chapter 5 | 93 | 3 | 4 | 12 | 112 | 5 |
| Chapter 6 | 47 | 3 | 1 | 0 | 51 | 28 |
| Chapter 7 | 75 | 5 | 7 | 0 | 87 | 10 |
| Chapter 8 | 110 | 2 | 1 | 10 | 123 | 27 |
| Chapter 9 | 71 | 1 | 1 | 43 | 116 | 26 |
| **Whole book** | **717** | **55** | **34** | **67** | **873** | **191** |

`UNSURE` is not exposed as a public README status: it is conservatively counted as `partial`. The six prose-only `UNSURE` rows with no Lean evidence remain blank in the README rather than being assigned a misleading declaration.

## Final proof and repository audit

- **Core build:** `~/.elan/bin/lake build HighDimensionalProbability` completed successfully
  with **3,508 jobs**. The target imports every consolidated chapter, Prelude module, and
  exercise aggregator while leaving the separately maintained appendix subtree outside the
  build boundary. Output contained linter/style warnings and the expected intentional
  exercise-sorry warnings, but no errors.
- **Declaration documentation:** `scripts/audit_docstrings.py --summary-only --check` audited
  **5,794 theorem/lemma/def declarations in 98 in-scope files**. All 5,794 have attached
  docstrings; missing-docstring, source-label, citation-shape, citation-order, and generic-opening
  issue counts are all zero. Separate scans found no invalid attribute/modifier placement, no
  docstring stranded above an `omit ... in` wrapper, and no remaining LaTeX/source labels.
- **Mathematics preservation:** `scripts/verify_declaration_preservation.py` passed all nine
  chapters. Baseline/consolidated declaration counts and declaration-multiset hashes are:
  Ch1 183/183 `9479f50488f0091d`; Ch2 307/307 `d3d69ec825833ef5`;
  Ch3 555/555 `3fdb43185e42084c`; Ch4 670/670 `7b7640a17e7f8849`;
  Ch5 511/511 `62eecf4c6e6641dd`; Ch6 324/324 `0b6282f053bf6d68`;
  Ch7 646/646 `48742eabec77c0be`; Ch8 1031/1031 `8789fffbefcf4ba2`;
  Ch9 859/859 `a3e4b1d7921ea89b`. Every former numbered source block also matched its
  consolidated code-token block exactly, modulo the four recorded private-name collisions.
- **Proof-token scan:** after stripping comments, the in-scope tree contains exactly **231
  `sorry` tokens**, all in `ChapterN/Exercise/` files and all intentional `EXERCISE-SORRY`
  leaves. The chapter distribution is 5, 44, 30, 34, 7, 32, 14, 34, and 31 for Chapters 1--9,
  identical to the pre-edit baseline. There are zero in-scope `admit`, `native_decide`, or
  `axiom` code tokens, zero core/Prelude sorries, and zero `UNRESOLVED-PROOF-NNN` markers.
- **Exhaustive proved-row axiom audit:** `scripts/verify_readme_axioms.py` resolved **573 unique
  Lean endpoints covering all 717 README rows marked `proved`**. No endpoint depends on
  `sorryAx` or a project-specific axiom. Axiom-set distribution: 1 with no axioms; 1 with
  `[Quot.sound]`; 2 with `[Quot.sound, propext]`; 569 with
  `[Classical.choice, Quot.sound, propext]`.
- **Master-map integrity:** the README has exactly **873** conclusion rows with status totals
  717 proved, 55 partial, 34 missing, and 67 in appendix. All 34 missing rows have blank Lean
  cells; every nonblank listed `.lean` path exists. Independent endpoint-to-file QA found no
  discrepancy in any partial or appendix row. REVIEW_NOTES contains exactly 89 gap obligations
  (45 PARTIAL, 34 MISSING, 10 UNSURE), 67 appendix rows, and 717 proved/stronger faithfulness
  rows (680 FORMALIZED, 37 STRONGER).
...
During the initial direct-import consolidation, every chapter was built before the next chapter began. Those successful core gates processed 3,643, 3,623, 3,597, 3,579, 3,565, 3,552, 3,539, 3,526, and 3,509 jobs for Chapters 1 through 9 respectively. Chapter 3 had one transient `.olean` race and passed on a clean retry. After the transitively imported Chapter 8 foundation was absorbed, the final full core regression processed 3,508 jobs successfully.

## Audit interpretation

- `FORMALIZED` and `STRONGER` become public status `proved` only after final axiom verification.
- `PARTIAL` remains `partial`; the exact missing component is stated row by row.
- `UNSURE` becomes `partial`, never `proved`.
- `MISSING` remains `missing`, with blank Lean columns in the README.
- `IN-APPENDIX` becomes `in appendix`; a conditional core wrapper is not treated as an unconditional proof of the advertised result.
- Non-load-bearing practice exercises and their `EXERCISE-SORRY` leaves are intentional and are not comprehensiveness gaps.

## Every partial, missing, and unsure obligation

This table has **89** rows: every 45 `PARTIAL`, 34 `MISSING`, and 10 
`UNSURE` obligation from the source audits. A missing row may mention nearby 
infrastructure as search evidence; it is not represented as a matching Lean theorem 
in the master map.

| Chapter | Book ref | Original class | Result | Located Lean evidence | Audited file evidence | Exact gap / uncertainty / faithfulness issue |
|---|---|---|---|---|---|---|
| Appetizer | Theorem 0.0.2 | MISSING | Approximate Caratheodory: an equal average of `k` points approximates any point of the convex hull within `1/sqrt(k)`. | — | — | Searches found neither an empirical-method theorem nor an equal-weight approximation wrapper. |
| Appetizer | Corollary 0.0.3 | MISSING | An `N`-vertex polytope in the unit ball has an `N^k`-point cover at radius `1/sqrt(k)`. | — | — | Later metric-entropy infrastructure defines nets, but does not prove this corollary. |
| Appetizer | Theorem 0.0.4; (0.2) | MISSING | The relative volume of an `N`-vertex polytope in the unit ball is at most `(3 sqrt(log N/n))^n`. | — | — | No Chapter-0 volume argument or wrapper was found. |
| Appetizer | (0.3) | MISSING | Before optimizing, the covering argument gives `Vol(P)/Vol(B) <= N^k/k^(n/2)`. | — | — | This intermediate volume/covering estimate is absent. |
| Appetizer | (0.4) | MISSING | The continuous optimizer is `k0 = n/(2 log N)`. | — | — | Elementary calculus alone exists upstream, but the source expression and optimization step are not packaged. |
| Appetizer | Remark 0.0.5 | MISSING | Polytopes with subexponentially many vertices occupy vanishingly small relative volume. | — | — | This substantive asymptotic conclusion depends on the missing Theorem 0.0.4. |
| Appetizer | Exercise 0.1(a) | MISSING | Vector bias-variance identity: `E ‖Z-EZ‖^2 = E ‖Z‖^2-‖EZ‖^2`. | — | — | The scalar identity is available through variance, but no exact finite-dimensional vector theorem was found. |
| Appetizer | Exercise 0.2 | PARTIAL | The mean minimizes expected squared distance. | `HDP.integral_sq_sub_eq_variance_add`; `HDP.variance_le_integral_sq_sub` | `HighDimensionalProbability/Prelude/Basic.lean` | The proved scalar statement is exactly what (2.23) later needs; the PDF asks for the stronger `R^n` version. |
| Appetizer | Exercise 0.3 | MISSING | Independent mean-zero random vectors satisfy `E ‖sum Z_j‖^2 = sum E ‖Z_j‖^2`. | — | — | Scalar variance-sum theorems exist in Chapter 1, but no exact vector-valued identity was found. |
| Chapter 1 | Markov optimality prose | MISSING | With only the mean known, Markov's bound is best possible. | — | — | No extremizing distribution theorem was found. |
| Chapter 1 | Notes: sharp Robbins/Stirling bounds | MISSING | Two-sided `sqrt(2 pi n)(n/e)^n exp(1/(12n+1)) <= n! <= ... exp(1/(12n))`. | — | — | The old inventory called this “historical”; the PDF states a substantive non-asymptotic theorem. Mathlib has related successive-difference bounds, but no exact source-facing endpoint was found. |
| Chapter 2 | Question 2.1.1; (2.1) | PARTIAL | Chebyshev gives only `P(S_N >= 3N/4) <= 4/N` for fair coin flips. | `HDP.Chapter1.chebyshev_inequality`; `variance_sample_mean`; Bernoulli/binomial moment API | `HighDimensionalProbability/Chapter1_AnalysisAndProbabilityRefresher.lean` | All ingredients are proved, but the exact `4/N` heads calculation has no declaration; `remark_2_2_4` proves the later exponential improvement, not (2.1). |
| Chapter 2 | (2.2) | UNSURE | CLT heuristic replaces the normalized coin sum by a standard Gaussian at a threshold growing with `N`. | `HDP.Chapter1.central_limit_theorem`; `deMoivreLaplace` | `HighDimensionalProbability/Chapter1_AnalysisAndProbabilityRefresher.lean` | UNSURE is conservatively published as partial. The PDF explicitly says this moving-threshold approximation is not justified by the ordinary CLT. |
| Chapter 2 | Remark 2.1.3 | UNSURE | More precise Mills-ratio expansions are available. | `HDP.Chapter2.gaussian_mills_identity` (supporting identity only) | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean` | UNSURE is conservatively published as partial. The remark itself only points to out-of-scope Exercise 2.3; the full alternating expansion/sandwich is not a proved endpoint. |
| Chapter 2 | Remark 2.2.3 | UNSURE | Concentration inequalities are non-asymptotic. | — | — | UNSURE is conservatively published as partial. Methodological prose, not a mathematical proposition. |
| Chapter 2 | Remark 2.3.3 | PARTIAL | Chernoff resembles a Poisson point mass; Poisson tails decay like `exp(-t log t)` and are heavier than Gaussian. | `HDP.Chapter1.poisson_pmf_asymptotic`; `HDP.Chapter2.chernoff_upper` | `HighDimensionalProbability/Chapter1_AnalysisAndProbabilityRefresher.lean`; `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean` | The two ingredients exist, but no theorem packages the tail-comparison/asymptotic prose. |
| Chapter 2 | Remark 2.3.5 | PARTIAL | Binomial/Poisson behavior is Gaussian near the mean and Poisson-like far away. | `HDP.Chapter2.chernoff_normalized`; `HDP.Chapter1.poisson_limit_theorem` (appendix) | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean`; `HighDimensionalProbability/Appendix/PoissonLimitTheorem.lean` | The normalized small-deviation inequality is proved; the plotted two-regime PMF comparison is expository and partly appendix-dependent. |
| Chapter 2 | Median robustness prose | MISSING | A single extreme outlier moves a median only to a neighboring order statistic. | — | — | The estimator theorem is proved, but this deterministic robustness claim has no endpoint. |
| Chapter 2 | Example 2.6.5 | PARTIAL | Normal, Rademacher, Bernoulli, binomial, and bounded laws are subgaussian; exponential/Poisson/geometric/chi-square/Gamma/Cauchy/Pareto are not. | `HDP.psi2Norm_standardGaussian`; `psi2Norm_rademacher`; `psi2Norm_bernoulli`; `psi2Norm_le_of_bounded`; sum closure | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean`; `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean` | Positive examples are covered (binomial by sum closure). The PDF's full negative list is only out-of-scope Exercise 2.25 and has no proved core theorem. The older inventory's blanket “proved” label is too strong. |
| Chapter 2 | Remark 2.7.2 | MISSING | A reverse bound holds for identically distributed summands but fails in general. | — | — | The claimed answers are deferred/skipped in out-of-scope Exercises 2.33--2.34; no core theorem was found. |
| Chapter 2 | Remark 2.8.2 | UNSURE | A centered unit-variance MGF is approximately Gaussian near zero by Taylor expansion. | — | — | UNSURE is conservatively published as partial. The PDF explicitly presents an approximation/intuition, not a quantified remainder theorem. |
| Chapter 2 | Example 2.8.7 | PARTIAL | Lists subexponential and non-subexponential named distributions. | `HDP.subGaussian_to_subExponential`; `HDP.subExponential_sq_iff`; `HDP.exponential_mgf_diverges` | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean`; `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean` | Generic implications and the exponential example are proved; Poisson/geometric/chi-square/Gamma membership and Cauchy/Pareto nonmembership lack exact endpoints. |
| Chapter 2 | Remark 2.9.3 | PARTIAL | Both Gaussian and exponential tails are necessary; one subexponential term may have an exponential tail. | `HDP.exponential_mgf_diverges`; `HDP.SubExponential.tail_bound` | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean`; `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean` | The class bound and canonical exponential-law obstruction exist, but no theorem packages the stated necessity/optimality conclusion. |
| Chapter 3 | Covariance prose | PARTIAL | `cov(X)=E[XX^T]-mu mu^T`; covariance is symmetric positive semidefinite; for mean zero it equals the second moment. | `HDP.covarianceMatrix_transpose`; `HDP.Chapter3.covarianceMatrix_transpose`; `ProbabilityTheory.isPosSemidef_covarianceBilin`; `HDP.secondMomentMatrix` | `HighDimensionalProbability/Prelude/RandomVector.lean`; `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `.lake/packages/mathlib/Mathlib/Probability/Moments/CovarianceBilin.lean` | Symmetry/PSD and both matrices exist, but no exact finite-matrix identity `cov=secondMoment-mu mu^T` or mean-zero corollary was found. |
| Chapter 3 | Corollary 3.2.3 | PARTIAL | PCA directions successively maximize variance of centered one-dimensional projections. | `HDP.Chapter3.secondMoment_pca_kth_maximum`; `pca_kth_maximum_principle` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Exact linear algebra and an uncentered second-moment random-vector wrapper exist; the printed covariance/variance wrapper for `X-EX` is missing. |
| Chapter 3 | Remark 3.2.4 | UNSURE | Keeping the leading principal components gives a practical dimension-reduction method. | — | — | UNSURE is conservatively published as partial. Algorithmic/interpretive prose; no error criterion or theorem is stated. |
| Chapter 3 | Corollary 3.3.3 | PARTIAL | A sum of independent normals is normal, with means and variances adding. | `HDP.Chapter3.sum_independent_gaussians_hasGaussianLaw` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Lean proves the more general vector-valued “sum is Gaussian” fact, but does not expose the printed scalar mean/summed-variance parameters in this wrapper. |
| Chapter 3 | Definition 3.3.4 | PARTIAL | A general Gaussian vector is an affine image `mu+A Z` of a standard Gaussian. | `HDP.HasGaussianVectorLaw`; Mathlib `multivariateGaussian` | `HighDimensionalProbability/Prelude/RandomVector.lean`; `.lake/packages/mathlib/Mathlib/Probability/Distributions/Gaussian/Multivariate.lean` | The local definition specifies a mean/covariance law; no source-facing equivalence with an arbitrary affine image is proved. |
| Chapter 3 | (3.14) | PARTIAL | Two independent isotropic vectors are almost orthogonal: inner product is `O(1/sqrt(n))` with high probability after normalization. | `HDP.Chapter3.secondMoment_independent_copy_inner_sq`; `HDP.Chapter1.markov_inequality` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `HighDimensionalProbability/Chapter1_AnalysisAndProbabilityRefresher.lean` | The exact second moment and Markov tools exist, but the displayed high-probability asymptotic wrapper is absent. |
| Chapter 3 | (3.17) | UNSURE | Informal heuristic `N(0,I_n) approximately Unif(sqrt(n) S^{n-1})`. | — | — | UNSURE is conservatively published as partial. The PDF explicitly labels this informal; (3.15), (3.16), and Theorem 3.3.9 are the rigorous substitutes. |
| Chapter 3 | (3.18) | PARTIAL | Quantitative CDF error decomposition used in the projective-CLT proof. | `HDP.Chapter3.projectiveRatio_uniform_cdf`; `standardGaussian_cdf_increment_le` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | The final uniform convergence and Gaussian interval bound are stronger endpoints, but the printed `p_n` decomposition is not exported verbatim. |
| Chapter 3 | Theorem 3.4.5; (3.19) | PARTIAL | Sphere marginal tail `P(<X,v>>=t) <= 2 exp(-n t^2/2)` and hence the sphere is subgaussian at scale `1/sqrt(n)`. | `HDP.Chapter3.sphere_tail`; `unitSphere_marginal_tail`; `unitSphere_subGaussian`; `psi2NormVector_unitSphere_le` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Qualitative theorem and dimension scaling are proved, but Lean uses the much weaker explicit denominator `(512 sqrt(2))^2`; the printed `1/2` exponent constant is not recovered. |
| Chapter 3 | (3.20) | MISSING | Conditioning identity for the Gaussian-ratio proof of the sphere tail. | — | — | Current Lean proves the tail by a different norm-concentration/ratio decomposition and does not export this conditional-probability equality. |
| Chapter 3 | (3.21) | MISSING | Product/Laplace-transform evaluation for the conditioned Gaussian norm. | — | — | The exact printed expectation/product calculation is absent; the endpoint uses a different proof. |
| Chapter 3 | Example 3.4.6 | PARTIAL | Uniform convex-body marginals: bounded bodies are subgaussian; Borell gives subexponential marginals for arbitrary convex bodies. | `HDP.bounded_to_subGaussian`; unit-ball/sphere infrastructure | `HighDimensionalProbability/Chapter2_ConcentrationOfIndependentSums.lean`; `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Only the generic scalar bounded implication exists. Borell's lemma and a convex-body vector wrapper are missing. The PDF's claim that `[-1,1]^n` is isotropic is false under Definition 3.2.5; the isotropic cube is scaled to `[-sqrt(3),sqrt(3)]^n`. |
| Chapter 3 | Example 3.4.7 | MISSING | Coordinate distribution has vector `psi_2` norm of order `sqrt(n/log n)`. | — | — | Only out-of-scope `Exercise/Sec04.exercise_3_43` has an intentional placeholder. |
| Chapter 3 | Example 3.4.8 | MISSING | Isotropic discrete subgaussian laws must have exponentially large support/entropy. | — | — | Only out-of-scope Exercise 3.46 has a placeholder; no proved main endpoint exists. |
| Chapter 3 | (3.25) | MISSING | Explicit `L^2` bound for a truncated Gaussian used in the first Grothendieck proof. | — | — | The current proof is the stronger Krivine route and contains no source-facing truncated-variable inequality. |
| Chapter 3 | (3.31) | PARTIAL | Graph cut cardinality equals the adjacency-matrix sign objective. | `HDP.cutMatrixObjective`; graph `cutSize` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `HighDimensionalProbability/Prelude/SimpleGraph.lean` | Both sides are defined, but no theorem connects `cutSize G s` to `cutMatrixObjective G.adjMatrix x`. |
| Chapter 3 | (3.32) | PARTIAL | `maxcut(G)` is the maximum of the sign-matrix objective. | `HDP.SimpleGraph.maxCutValue`; `HDP.cutMatrixObjective`; `HDP.exists_integerObjective_maximizer` | `HighDimensionalProbability/Prelude/SimpleGraph.lean`; `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Missing the graph-to-matrix bridge from (3.31), so the equality of the two maxima is not exposed. |
| Chapter 3 | Proposition 3.6.3 | PARTIAL | A uniformly random cut has expected size at least half of maximum cut. | `HDP.Chapter3.randomCut_expected_objective`; `randomCut_halfApproximation` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Strong weighted-matrix objective theorem, but the exact graph-cardinality corollary depends on the missing (3.31) bridge. |
| Chapter 3 | (3.33) | PARTIAL | Max-cut SDP objective/maximization. | `HDP.sdpCutObjective`; `HDP.vectorSDPObjective_attains` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | The objective and generic attainment exist; no graph-named `sdp(G)` maximum is defined. |
| Chapter 3 | Theorem 3.6.4 | PARTIAL | Goemans--Williamson rounding achieves at least `.878 sdp(G)` and hence `.878 maxcut(G)` in expectation. | `HDP.Chapter3.goemans_williamson_expected_guarantee`; `cut_embedding_preserves_objective`; `gaussian_rounding_expected_objective` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Exact `439/500` weighted-objective rounding guarantee and SDP relaxation embedding are proved; graph-level `sdp(G)`/`maxcut(G)` wrappers still depend on missing (3.31)--(3.33) bridges. |
| Chapter 3 | Definition 3.7.1; (3.36) | PARTIAL | General order-`k` tensors and their coordinate inner product. | `HDP.TensorPowerSpace`; ambient `PiLp` inner product | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Lean implements equal-axis tensor powers sufficient for the kernel proof, not the PDF's fully general `n_1 x ... x n_k` tensor type; (3.36) is inherited rather than wrapped. |
| Chapter 3 | Example 3.7.2 | PARTIAL | Vectors and matrices are order-one and order-two tensors. | `HDP.TensorPowerSpace` specializations | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Definitional special cases exist, but there are no named vector/matrix identification theorems. |
| Chapter 3 | Lemma 3.7.7 | PARTIAL | Entire real power series has a Hilbert feature-map realization. | `HDP.Chapter3.realAnalytic_featureMap`; `realAnalytic_featureMap_norm`; `HDP.AbsolutePowerSummable` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Lean assumes absolute power summability directly; it does not prove the bridge from the PDF's hypothesis “the series converges for every real `x`.” |
| Chapter 3 | Remark 3.7.9 | PARTIAL | Krivine's proof gives a randomized rounding algorithm with an expected bilinear guarantee. | `HDP.Chapter3.grothendieck_inequality`; private compiled `integral_krivine_rounding` proof step | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | The deterministic theorem and its probabilistic proof are compiled, but no public algorithm/output or expectation wrapper matching the remark is exposed. |
| Chapter 3 | (3.38); Moore--Aronszajn prose | PARTIAL | A kernel has a Hilbert feature map iff it is positive semidefinite. | `HDP.Chapter3.moore_aronszajn`; Mathlib `RKHS.OfKernel.kernel_ofKernel` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean`; `.lake/packages/mathlib/Mathlib/Analysis/InnerProductSpace/Reproducing.lean` | Sufficiency is exposed for operator-valued kernels; the elementary necessity/iff wrapper for scalar kernels is not packaged locally. |
| Chapter 3 | Notes after Theorem 3.1.1 | MISSING | Dependence on `K` improves from `K^2` to the optimal `K sqrt(log K)`. | — | — | Current Lean proves the book theorem's `K^2` bound only. |
| Chapter 3 | Notes on Krivine constant | UNSURE | The true real Grothendieck constant is strictly below the explicit Krivine value, but no explicit expression is known. | — | — | UNSURE is conservatively published as partial. External state-of-knowledge statement, not a fixed theorem formulated in the project. |
| Chapter 3 | Notes on Goemans--Williamson | UNSURE | `.878...` is the best known approximation ratio; conditional UGC hardness rules out improvement. | — | — | UNSURE is conservatively published as partial. Complexity/history claim with a conjectural condition; outside theorem-level probability formalization. |
| Chapter 3 | Notes improving Exercise 3.13 | PARTIAL | For standard Gaussians, `E max_i ‖X_i‖ <= sqrt(n)+sqrt(2 log N)` (the PDF prints `n` in one extraction location, but context is number of samples). | `HDP.Chapter3.exercise_3_13a` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Correct `sqrt(n)+C sqrt(log N)` scale is proved, but not the sharp `sqrt(2 log N)` constant. |
| Chapter 3 | Notes on PSD Grothendieck | MISSING | The constant `pi/2` is optimal for PSD Grothendieck. | — | — | Exercise 3.57 is an out-of-scope placeholder and even its upper bound does not establish optimality. |
| Chapter 3 | Exercise 3.55 | PARTIAL | Analytic-kernel feature-map construction. | `HDP.Chapter3.realAnalytic_featureMap`; `realAnalytic_featureMap_norm` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | Construction is proved under `AbsolutePowerSummable`; the bridge from pointwise convergence of the entire power series is absent, as for Lemma 3.7.7. |
| Chapter 4 | Remark 4.1.3 | PARTIAL | Zero-pad/order singular values and write `A = U Sigma V^T` using square orthogonal matrices. | `HDP.Chapter4.singularValue_eq_zero_of_domain_le`<br>`HDP.Chapter4.singularValueDecomposition_linearMap`<br>`HDP.Chapter4.RealSVD` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | Zero padding and an equivalent rank-one/linear-map decomposition are proved, but no public declaration produces the literal square orthogonal `U,Sigma,V` factorization. |
| Chapter 4 | Example 4.1.5 | MISSING | A rank-`k` orthogonal projection is `UU^T`, symmetric, with `k` eigenvalues one and the rest zero. | — | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | The previous inventory credited generic SVD infrastructure, but that is not a declaration of this substantive worked example. |
| Chapter 4 | Remark 4.1.9 | PARTIAL | General `p -> q` induced matrix norms and the bilinear dual formula. | `HDP.Chapter4.matrixLpToLpNorm`<br>`HDP.Chapter4.matrixLpToLpNorm_apply_le`<br>`HDP.Chapter4.matrixLpToLpNorm_ratio_le` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | The induced norm is defined and bounded correctly, but the general finite-dimensional maximum/attainment and full bilinear-max equality are not exposed; see equation (4.31). |
| Chapter 4 | Remark 4.2.13 | PARTIAL | Euclidean nets can be built by a scaled lattice or by random points. | `HDP.Chapter4.exercise_4_39`<br>`HDP.Chapter4.Exercise.exercise_4_31` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean`<br>`HighDimensionalProbability/Chapter4/Exercise/Sec02.lean` | Random-net half is proved; lattice half is only a non-load-bearing `EXERCISE-SORRY`. If the remark is counted as substantive main text, it is not fully proved. |
| Chapter 4 | Eq. (4.4) | PARTIAL | Square matrix form `A = U Sigma V^T`. | `HDP.Chapter4.singularValueDecomposition_linearMap` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | No literal square orthogonal-factor declaration; see Remark 4.1.3. |
| Chapter 4 | Exercise Eq. (4.31) | PARTIAL | General `p -> q` norm equals both a maximum ratio and a bilinear maximum. | `HDP.Chapter4.matrixLpToLpNorm`<br>`HDP.Chapter4.matrixLpToLpNorm_ratio_le`<br>`HDP.Chapter4.exercise_4_18c_duality` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | Definition, upper bound, norm axioms, and transpose duality are proved; general attainment and the displayed bilinear-max equality are not exposed. |
| Chapter 4 | Exercise 4.18 | PARTIAL | General `p -> q` induced matrix norm, its norm axioms, and transpose duality. | `HDP.Chapter4.matrixLpToLpNorm`<br>`HDP.Chapter4.exercise_4_18b_matrixLpToLpNorm`<br>`HDP.Chapter4.exercise_4_18c_duality` | `HighDimensionalProbability/Chapter4_RandomMatrices.lean` | The displayed max/bilinear-max identity (4.31), especially general attainment, is not fully exposed. |
| Chapter 5 | Definition 5.1.1 | PARTIAL | A map is Lipschitz if distances expand by at most a constant; `‖f‖_Lip` is the least such constant. | `LipschitzWith`<br>`LipschitzOnWith` | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean` | The predicate is authoritative, but the book's least-constant seminorm `‖f‖_Lip` is not defined as a Lean object. PDF also states the false global implication “differentiable implies Lipschitz”; Lean only proves the valid uniformly bounded-derivative version. |
| Chapter 5 | Example 5.1.2 | PARTIAL | Linear functionals, matrices, and norm maps have the advertised exact Lipschitz constants. | `HDP.Chapter5.Exercise.exercise_5_2a`<br>`_5_2b`<br>`_5_2c` | `HighDimensionalProbability/Chapter5/Exercise/Sec01.lean` | All three proofs are `EXERCISE-SORRY`; the PDF reverses the domain/codomain of an `m x n` matrix and Lean corrects it. This is a substantive numbered example even though its checking exercise is non-load-bearing. |
| Chapter 5 | Remark 5.4.6 | PARTIAL | Matrix monotonicity: inverse is antitone and logarithm monotone on positive definite matrices; scalar monotonicity does not suffice generally. | `HDP.Chapter5.exercise_5_17a_commutingMatrixFunction_monotone`<br>`inverse_loewner_antitone`<br>`logarithm_loewner_monotone` | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean` | Positive results are proved and the PDF's inverse direction/domain are corrected. No Lean counterexample proves the substantive negative claim that ordinary scalar monotonicity need not lift to matrices. |
| Chapter 5 | Eq. (5.2) | MISSING | Spherical isoperimetry implies `sigma(A_t) >= sigma(H_t)` for a half-measure set and hemisphere. | `HDP.Chapter5.spherical_isoperimetric` | `HighDimensionalProbability/Appendix/SphericalIsoperimetric.lean` | The older audit called this “audited proof-local,” but the displayed specialization is not Lean-checked anywhere. |
| Chapter 5 | Eq. (5.3) | MISSING | The hemisphere neighborhood contains the cap `{x_1 <= t/sqrt 2}`. | — | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean` | The alternate concentration-to-blow-up proof never formalizes this geometric containment. |
| Chapter 5 | pp.145--146, sphere proof | MISSING | A real random variable has a median satisfying both half-probability inequalities. | `HDP.Chapter5.IsMedian` | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean` | This hidden existence obligation is bypassed by the alternate proof of sphere concentration. |
| Chapter 5 | p.163 before Golden--Thompson | MISSING | In general `exp(X+Y) != exp(X)exp(Y)` for noncommuting symmetric matrices. | `HDP.Chapter5.matrixExponential_add_of_commute` | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean` | The counterexample prompt in Exercise 5.19(b) is practice-only, but the negative assertion is substantive main-text prose. |
| Chapter 6 | Remark 6.5.2 | PARTIAL | Matrix completion extends to rectangular/noisy data and can be improved to remove the log or achieve exact noiseless recovery. | `HDP.Chapter6.exercise_6_31_rectangular_matrix_completion`<br>`exercise_6_32_noisy_matrix_completion` | `HighDimensionalProbability/Chapter6_QuadraticFormsSymmetrizationContraction.lean` | The PDF gives no target formulas for Exercises 6.31--6.32, so correspondence is not unique. No declaration captures the separate qualitative log-free or exact-recovery claims. |
| Chapter 6 | Remark 6.5.2 qualitative tail | MISSING | The logarithmic factor can be removed and noiseless observations can achieve exactly zero error. | — | `HighDimensionalProbability/Chapter6_QuadraticFormsSymmetrizationContraction.lean` | The PDF gives no hypotheses/formula and points only to notes; nevertheless these are substantive existence/improvement claims. |
| Chapter 6 | Exercise 6.31 | UNSURE | A rectangular extension of matrix completion. | `HDP.Chapter6.exercise_6_31_rectangular_matrix_completion` | `HighDimensionalProbability/Chapter6_QuadraticFormsSymmetrizationContraction.lean` | UNSURE is conservatively published as partial. PDF merely says “state and prove a version”; there is no unique source statement against which to verify the chosen Lean theorem. |
| Chapter 6 | Exercise 6.32 | UNSURE | A noisy-observation extension of matrix completion. | `HDP.Chapter6.exercise_6_32_noisy_matrix_completion` | `HighDimensionalProbability/Chapter6_QuadraticFormsSymmetrizationContraction.lean` | UNSURE is conservatively published as partial. PDF says “extend” without a target rate/normalization/noise law, so exact correspondence is indeterminate. |
| Chapter 7 | Example 7.1.2 | MISSING | A process indexed by `{1,...,n}` is the same data as a random vector. | — | — | Numbered explanatory example was skipped; no declaration packages the identification. |
| Chapter 7 | Example 7.1.3 | MISSING | A random walk is the partial-sum process of independent mean-zero increments. | — | — | No random-walk definition or partial-sum endpoint in Chapter 7. |
| Chapter 7 | Example 7.1.4 | MISSING | Standard Brownian motion has continuous paths and independent Gaussian increments. | — | — | No Brownian model. The printed "characterization" also omits `X_0=0` a.s. and joint independence for disjoint increments. |
| Chapter 7 | Example 7.1.5 | MISSING | A process indexed by a subset of Euclidean space is a random field. | — | — | Terminological example only; no declaration. |
| Chapter 7 | Example 7.1.6 | MISSING | Brownian-motion and unit-variance random-walk increments have square-root distance. | — | — | No Brownian/random-walk model exists from which these identities can be stated. |
| Chapter 7 | Theorem 7.1.11 | PARTIAL | The centered maximum of a finite Gaussian process has `psi2` norm bounded by a universal constant times the largest marginal standard deviation. | `HDP.Chapter7.finiteGaussianProcess_concentration` | `HighDimensionalProbability/Chapter7_RandomProcesses.lean` | The declaration proves the canonical affine-representation case. Lemma 7.1.12 shows such a representation exists, but there is no source-facing wrapper transporting the result to an arbitrary finite Gaussian process. |
| Chapter 7 | Proposition 7.5.2 | PARTIAL | Gaussian width is finite for bounded sets and obeys scaling, Minkowski, convex-hull, symmetry, diameter, and linear-image laws. | `HDP.Chapter7.gaussianWidth_scale`; `gaussianWidth_minkowskiSum`; `gaussianWidth_convexHull`; `gaussianWidth_diameter_bounds`; `exercise_7_15g_linearImage` | `HighDimensionalProbability/Chapter7_RandomProcesses.lean` | All finite-set laws are proved, and later arbitrary-set convex-hull/scaling lemmas exist, but there is no single arbitrary-bounded-set API covering every printed part (a)--(g). |
| Chapter 7 | Example 7.5.8 | PARTIAL | The cross-polytope has Gaussian width of order `sqrt(log n)`. | `HDP.Chapter7.crossPolytopeGaussianWidth_upper`; `HDP.Chapter7.crossPolytopeGaussianWidth_asymptotic` | `HighDimensionalProbability/Chapter7_RandomProcesses.lean` | A safe upper bound and iid asymptotic statement are proved, but no finite two-sided constant-comparison theorem states the displayed `asymp` directly. |
| Chapter 7 | Remark 7.5.10 | MISSING | In high dimensions a set can have huge diameter but small width, and width can behave unexpectedly across examples. | — | — | Numbered substantive synthesis is only prose; no theorem packages its comparison/negative claim. |
| Chapter 7 | Eq. (7.19) | PARTIAL | Cross-polytope width is comparable to `sqrt(log n)`. | `HDP.Chapter7.crossPolytopeGaussianWidth_upper`; `crossPolytopeGaussianWidth_asymptotic` | `HighDimensionalProbability/Chapter7_RandomProcesses.lean` | Missing direct finite two-sided wrapper; `n=1` also needs a corrected range. |
| Chapter 7 | Eq. (7.21) | PARTIAL | Projecting the Euclidean ball to a nonzero subspace does not shrink its diameter. | no standalone equation endpoint; projection infrastructure in `randomProjection_expectedDiameter` | `HighDimensionalProbability/Chapter7_RandomProcesses.lean` | The exact ball identity is only an explanatory special case inside the corrected model. |
| Chapter 7 | Sec. 7.2.1 Brownian reflection example | MISSING | `E sup_{t<=t0} B_t = sqrt(2t0/pi)`. | — | — | Substantive unnumbered conclusion; no Brownian model exists. |
| Chapter 8 | Example 8.1.2 | PARTIAL | Every Gaussian process is subgaussian for its canonical `L2` pseudometric; any process is trivially so for its `psi2` increment metric. | `HDP.Chapter7.canonicalGaussianProcess_hasSubGaussianIncrementsWith`; `HDP.Chapter7.canonicalGaussianProcess_hasSubGaussianIncrements` | `HighDimensionalProbability/Chapter8_Chaining.lean` | Only the finite-dimensional canonical Gaussian representation is packaged. No arbitrary Gaussian-process transport wrapper or theorem for the trivial `psi2` metric is exposed. |
| Chapter 8 | Remark 8.2.4 | PARTIAL | One sample realizes the uniform Lipschitz error bound simultaneously for every function. | `HDP.Chapter8.remark_8_2_4_exists_good_lipschitz_sample_finite_grid` | `HighDimensionalProbability/Chapter8_Chaining.lean` | The named existence theorem covers a finite grid and finite function family. The arbitrary full Lipschitz class is controlled in expectation by 8.2.3, but no matching arbitrary-class existence wrapper is declared. |
| Chapter 8 | Sec. 8.1 after Example 8.1.2 | MISSING | Any process becomes subgaussian-increment if distance is defined by its increment `psi2` norm. | — | — | Substantive unnumbered claim; follows tautologically when norms are finite, but no declaration states it. |
| Chapter 9 | Remark 9.6.5 | MISSING | Extending the general matrix-deviation theorem to subgaussian matrices is open. | — | — | Correct disposition: this is explicitly an open problem, so the core documents it without asserting a theorem or axiom. |
| Chapter 9 | Sec. 9.2.3, proof of Theorem 9.2.2 | PARTIAL | For the covariance ellipsoid, radius is the square root of operator norm and Gaussian complexity is at most the square root of trace. | `HDP.Chapter9.factorEllipsoidNet_radius_le`; `factorEllipsoidNet_gaussianComplexity_le` | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean` | Lean proves the finite quarter-net factor versions used by the final theorem, but exposes no standalone actual-ellipsoid pair of identities at the PDF's scope. |

## Appendix-owned coverage: not a core proof claim

The following **67** audit rows are covered only under the 
`in appendix` status. They may name checked core wrappers, but those wrappers can be 
conditional on an appendix-owned principle. Nothing in this section asserts that an 
`EXTERNAL-SORRY`, custom assumption, or its transitive dependents are kernel-proved.
...
## Row-level faithfulness, source, and citation notes on proved/stronger rows

The following exhaustive table retains the note from every one of the **717** 
proved/stronger rows. Exact/faithful rows remain visible so no source repair, 
strengthened form, extra hypothesis, boundary convention, dimension/range correction, 
or citation nuance is lost to a heuristic filter. These notes do not downgrade a row.
...
| Chapter 3 | Remark 3.1.2 | FORMALIZED | Thin-shell phenomenon and bounded radial variance follow from norm concentration. | The main tail and its variance consequence are both proved; the old inventory's “missing (expository)” label is stale. |
...
| Chapter 3 | Definition 3.5.4; (3.26) | FORMALIZED | Generic semidefinite-program schema: linear matrix objective, PSD variable, linear constraints. | The stale inventory's “schema missing” statement is no longer true. |
...
| Chapter 3 | Exercise 3.13 | FORMALIZED | Expected maximum norm of subgaussian/Gaussian vectors. | Correct scale; the sharper notes-only Gaussian constant remains partial above. |
...
| Chapter 5 | Exercise 5.6 | FORMALIZED | Mean- and median-centered psi2 norms control each other. | Requires a supplied `IsMedian`; existence remains a separate missing prose obligation. |
...
| Chapter 5 | Exercise 5.17 | FORMALIZED | Commuting symmetric matrices preserve scalar functional-calculus order. | Noncommuting counterexample subpart is construction-only; its main-text negative claim remains missing above. |
...
| Chapter 5 | Exercise 5.19 | FORMALIZED | Commuting matrix exponentials multiply. | Pure noncommuting witness is out of scope; see missing main-prose row. |
