...
non-load-bearing exercise obligations. The final semantic audit then demotes
five proved-but-unfaithful core rows to `PARTIAL`. The resulting mathematical
census has **838 valid conclusions**: **763 core-formalized**, **5
core-partial**, **65 appendix-proved**, **5 appendix-unresolved/deferred**, and
**0 missing**. The **191** wholly non-load-bearing practice exercise numbers
remain outside the census.

| Chapter | Frozen input rows | Core formalized | Core partial | Appendix proved | Appendix unresolved / deferred | Rejected after PDF revalidation | Valid conclusions | Practice exercise numbers out of scope |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Appetizer | 11 | 11 | 0 | 0 | 0 | 0 | 11 | 6 |
| Chapter 1 | 63 | 62 | 0 | 1 | 0 | 0 | 63 | 6 |
| Chapter 2 | 79 | 70 | 0 | 1 | 0 | 8 | 71 | 29 |
| Chapter 3 | 111 | 93 | 3 | 0 | 1 | 14 | 97 | 31 |
| Chapter 4 | 120 | 117 | 0 | 0 | 0 | 3 | 117 | 23 |
| Chapter 5 | 112 | 99 | 0 | 12 | 1 | 0 | 112 | 5 |
| Chapter 6 | 51 | 47 | 0 | 0 | 0 | 4 | 47 | 28 |
| Chapter 7 | 87 | 80 | 1 | 0 | 1 | 5 | 82 | 10 |
| Chapter 8 | 123 | 112 | 1 | 8 | 2 | 0 | 123 | 27 |
| Chapter 9 | 116 | 72 | 0 | 43 | 0 | 1 | 115 | 26 |
| **Whole book** | **873** | **763** | **5** | **65** | **5** | **35** | **838** | **191** |
...
- The **PDF-revalidated census** reclassifies the 89 old gap rows as
  ...
  frozen 717 proved and 67 appendix rows, this produced the correction pass's
  intermediate **768 formalized / 0 partial / 0 missing / 70 appendix** split.
- The **final semantic audit** finds five cleanly compiled but unfaithful or
  incomplete core captures: Exercises 3.18–3.20, Remark 7.2.1, and equation
  (8.37). It therefore gives the final **763 core-formalized / 5 core-partial /
  65 appendix-proved / 5 appendix-unresolved/deferred** split above.
- The public **README correspondence table is curated for publication** and
  contains selected book-to-Lean mappings. Its row count is **611**:
  **609 verified and 2 marked partial**. It must not be compared directly with
  either the 873 audit-input rows or the 838 valid conclusions.

The pre-final contents of this file are preserved verbatim in
`REVIEW_NOTES.pre-final.md`. The complete severity-ordered defect catalogue,
appendix proof plans, and final verification evidence are in
...
- **Final-pass frozen-source check:** all **101** non-appendix source Lean
  files plus the package root module are byte-identical to the start snapshot
  (**102 compared, 0 different**). The requested declaration-preservation
  script was also rerun, but its baseline mode requires the older
  pre-consolidation numbered-module layout and therefore is not applicable to
  this already consolidated snapshot.
...
- **Whole-source token classification:** the isolated appendix has exactly one
  `sorry` (Gaussian Chevet) and no `admit`, `axiom`, `unsafe`, or
  `native_decide`. The 30 ordinary `decide` tokens split as 21 core/Prelude,
  4 exercise, and 5 appendix occurrences; all are ordinary kernel-checked
  finite/arithmetic decisions or Boolean encodings.
- **Exhaustive published-row axiom audit:** **534** unique Lean endpoints cover
  all **611** published rows. Of these, 530 depend on exactly
...
- **Appendix boundary:** `HighDimensionalProbability/Appendix.lean` and
  `HighDimensionalProbability/Appendix/**` remain intentionally outside the
  package root and core consolidation. The final appendix build and all 15
  target-specific axiom checks were run separately. Thirteen targets are
  axiom-clean proofs; Gaussian Chevet is `APPENDIX-UNRESOLVED-001`, and the
  positive-Ricci geometric bridge is `APPENDIX-UNRESOLVED-002`.
...
| Chapter | Book ref | Final status and reason for appendix ownership |
|---|---|---|
| Chapter 3 | Example 3.4.6 | **UNRESOLVED/DEFERRED.** The remaining arbitrary-convex-body `ψ₁` statement is attributed to Borell/Brunn–Minkowski without proof and is unused downstream; the bounded-body part is core-formalized and the isotropic cube requires `√3` scaling. |
| Chapter 5 | Eq. (5.2) | **APPENDIX-PROVED.** It is a specialization of the now proved isolated spherical-isoperimetry theorem. |
| Chapter 7 | Sec. 7.2.1 Brownian reflection example | **UNRESOLVED/DEFERRED.** The expectation identity uses the external reflection principle, with proof assigned to non-load-bearing Exercise 7.2, and is unused downstream. |
...
| Chapter 3 | Definition 3.3.4 | FORMALIZED | A general Gaussian vector is an affine image `mu+A Z` of a standard Gaussian. | `HDP.Chapter3.affineGaussianMeasure`; `affineGaussianMeasure_eq_multivariateGaussian`; `hasGaussianVectorLaw_iff_affineRepresentation` | `HighDimensionalProbability/Chapter3_RandomVectorsInHighDimensions.lean` | The raw `HasGaussianVectorLaw` predicate accepts non-PSD parameters, for which Mathlib totalizes the law as Dirac. Coverage uses the PSD-restricted affine-representation theorem, which constructs covariance `AAᵀ`. |
...
| Chapter 7 | Sec. 7.2.1 Brownian reflection example | IN-APPENDIX | `E sup_{t<=t0} B_t = sqrt(2t0/pi)`. | — | — | The source invokes the external reflection principle and assigns its proof to non-load-bearing Exercise 7.2; this unused result is deferred at the isolated appendix boundary. |
...
appendix-owned count is **70**. Final status within that isolated bucket is
**65 appendix-proved** and **5 unresolved/deferred**:

- unresolved Borell convex-body `ψ₁` (Example 3.4.6);
- unresolved positive-Ricci geometry bridge (equation (5.8),
  `APPENDIX-UNRESOLVED-002`);
- unresolved Brownian reflection identity;
- two sharp arbitrary-set Gaussian Chevet rows
  (`APPENDIX-UNRESOLVED-001`).

The majorizing-measure lower witness is now genuinely proved, so its Chapter
8–9 conditional wrappers discharge when the isolated appendix is imported.
They remain “appendix-owned” rather than “core-formalized” because the package
root deliberately does not import the appendix. Every Chapter 9 row in the
table therefore has final isolated status **APPENDIX-PROVED**; “conditional”
below describes only the core wrapper's explicit principle parameter, which
the axiom-clean appendix witness discharges.

| Chapter | Book ref | Result | Appendix/conditional evidence | File evidence | Final isolated status / notes |
|---|---|---|---|---|---|
| Chapter 1 | Theorem 1.7.6 | Poisson limit theorem for rare Bernoulli triangular arrays. | `HDP.Chapter1.poisson_limit_theorem` | `HighDimensionalProbability/Appendix/PoissonLimitTheorem.lean` | **APPENDIX-PROVED**, axiom-clean and intentionally isolated from the root. |
| Chapter 2 | Theorem 2.1.4 | Berry--Esseen quantitative CLT. | `HDP.Chapter2.berryEsseen` | `HighDimensionalProbability/Appendix/BerryEsseen.lean` | **APPENDIX-PROVED**, including the constant-one quantitative certificate. |
| Chapter 5 | Theorem 5.1.4 | Euclidean balls minimize boundary area and every epsilon-neighborhood volume at fixed volume. | `HDP.Chapter5.euclidean_isoperimetric` | `HighDimensionalProbability/Appendix/EuclideanIsoperimetric.lean` | **APPENDIX-PROVED** in the corrected nondegenerate measurable-neighborhood form. The PDF itself derives the boundary clause by a limit. |
| Chapter 5 | Theorem 5.1.5 | Spherical caps minimize epsilon-neighborhood area at fixed spherical area. | `HDP.Chapter5.spherical_isoperimetric` | `HighDimensionalProbability/Appendix/SphericalIsoperimetric.lean` | **APPENDIX-PROVED** in the corrected positive-cap-mass, closed chordal-neighborhood form. |
| Chapter 5 | Theorem 5.2.2 | Half-spaces minimize Gaussian epsilon-neighborhood measure at fixed Gaussian measure. | `HDP.Chapter5.gaussian_isoperimetric` | `HighDimensionalProbability/Appendix/GaussianIsoperimetric.lean` | **APPENDIX-PROVED** in the printed closed-expansion form. |
| Chapter 5 | Theorem 5.2.5 | Lipschitz functions on the normalized Hamming cube concentrate at scale `1/sqrt n`. | `HDP.Chapter5.hamming_cube_concentration` | `HighDimensionalProbability/Appendix/HammingCubeConcentration.lean` | **APPENDIX-PROVED** with corrected `n>0` and normalized distance. |
| Chapter 5 | Theorem 5.2.6 | Lipschitz functions on a uniform random permutation concentrate at scale `1/sqrt n`. | `HDP.Chapter5.symmetric_group_concentration` | `HighDimensionalProbability/Appendix/SymmetricGroupConcentration.lean` | **APPENDIX-PROVED** with corrected `n>0`. |
| Chapter 5 | Theorem 5.2.7 | Concentration for canonical Haar measure on `SO(n)`. | `HDP.Chapter5.special_orthogonal_concentration` | `HighDimensionalProbability/Appendix/SpecialOrthogonalConcentration.lean` | **APPENDIX-PROVED** independently of the unresolved general positive-Ricci bridge; uses canonical Borel Haar. |
| Chapter 5 | Theorem 5.2.9 | Lipschitz functions on a random Grassmannian subspace concentrate at scale `1/sqrt n`. | `HDP.Chapter5.Grassmannian`<br>`grassmannHaarMeasure`<br>`grassmannDistance`<br>`HDP.Chapter5.grassmannian_concentration` | `HighDimensionalProbability/Chapter5_ConcentrationWithoutIndependence.lean`<br>`HighDimensionalProbability/Appendix/GrassmannianConcentration.lean` | **APPENDIX-PROVED** through the corrected projection-orbit model and the proved `SO(n)` theorem. |
| Chapter 5 | Theorem 5.2.11 | Strongly convex exponential densities yield dimension-free concentration. | `HDP.Chapter5.strongly_convex_density_concentration` | `HighDimensionalProbability/Appendix/StronglyConvexDensity.lean` | **APPENDIX-PROVED** with normalized density and corrected Gaussian potential. |
| Chapter 5 | Theorem 5.2.12 | Talagrand convex concentration for product measures with bounded independent coordinates. | `HDP.Chapter5.talagrand_convex_concentration` | `HighDimensionalProbability/Appendix/TalagrandConvexConcentration.lean` | **APPENDIX-PROVED** on the exact cube with local convexity/Lipschitz hypotheses. |
| Chapter 5 | Theorem 5.7.1 | McDiarmid/bounded differences inequality for independent coordinates. | `HDP.Chapter5.bounded_differences` | `HighDimensionalProbability/Appendix/BoundedDifferences.lean` | **APPENDIX-PROVED** on the corrected dependent product domain. |
| Chapter 5 | Eq. (5.7) | Hamming-cube concentration inequality. | `HDP.Chapter5.hamming_cube_concentration` | `HighDimensionalProbability/Appendix/HammingCubeConcentration.lean` | **APPENDIX-PROVED.** |
| Chapter 5 | Eq. (5.8) | Positive-Ricci manifolds have Gaussian concentration at scale `1/sqrt kappa`. | `HDP.Chapter5.positive_ricci_concentration` | `HighDimensionalProbability/Appendix/PositiveRicciConcentration.lean` | **`APPENDIX-UNRESOLVED-002`.** The Bakry--Émery implication from a diffusion/Bochner certificate is proved, but no bridge constructs that certificate from an actual compact Riemannian manifold, normalized volume, and Ricci lower bound. |
| Chapter 8 | Theorem 8.5.5 | Gaussian expected suprema are equivalent, up to universal constants, to `gamma2`. | core upper `HDP.Chapter8.genericChainingBound`; conditional lower `majorizingMeasure_lower`; witness `HDP.Chapter8.majorizingMeasureLowerPrinciple_external` | `HighDimensionalProbability/Chapter8_Chaining.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | **APPENDIX-PROVED.** The difficult universal lower principle now has a constructive axiom-clean witness. |
| Chapter 8 | Corollary 8.5.6 | Talagrand comparison: a subgaussian process is controlled by a Gaussian process with dominating canonical increments. | `HDP.Chapter8.talagrandComparison`; `HDP.Chapter8.majorizingMeasureLowerPrinciple_external` | `HighDimensionalProbability/Chapter8_Chaining.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | **APPENDIX-PROVED:** the explicit principle parameter is discharged by the proved isolated witness. |
| Chapter 8 | Corollary 8.5.8 | Geometric Talagrand comparison bounds a subgaussian process by Gaussian width. | `HDP.Chapter8.corollary_8_5_8_geometric`; `HDP.Chapter8.majorizingMeasureLowerPrinciple_external` | `HighDimensionalProbability/Chapter8_Chaining.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | **APPENDIX-PROVED** through the same witness. |
...
| Chapter 9 | Remark 9.2.5 | The additive JL error is small once the target dimension dominates effective dimension. | `HDP.Chapter9.remark_9_2_5_setAdditiveJLError_le`; `remark_9_2_5_effectiveDimension_set`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | The numerical conclusion is packaged through the same appendix-proved principle constant; PDF's `T`/`X` inconsistency is corrected. |
...
| Chapter 9 | Example 9.3.2 | A random high-dimensional section of the cross-polytope has diameter `O(sqrt(log n/n))`. | `HDP.Chapter9.example_9_3_2_crossPolytopeBody_mStar`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional consequence. PDF's unrelated `T` in (9.13) is corrected to the cross-polytope body. |
| Chapter 9 | Remark 9.3.3 | The M-star bound is nontrivial once codimension dominates effective dimension. | `HDP.Chapter9.remark_9_3_3_effectiveDimension_set`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual-set conditional endpoint. |
| Chapter 9 | Theorem 9.3.4 | A random kernel avoids a spherical set once `m` dominates its squared width. | `HDP.Chapter9.theorem_9_3_4_escape`; `HDP.Chapter8.majorizingMeasureLowerPrinciple_external` | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Arbitrary spherical-set scope, but the unconditional bound inherits the appendix-proved lower principle. |
| Chapter 9 | Theorem 9.4.4 | Constrained recovery over a bounded prior has expected error `O(K^2 w(T)/sqrt m)`. | `HDP.Chapter9.theorem_9_4_4_constrainedRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Uses the actual uncountable prior and rigorous selectors, but is conditional on the appendix-proved lower principle. |
| Chapter 9 | Remark 9.4.5 | Constrained recovery becomes accurate above the prior's effective dimension. | `HDP.Chapter9.remark_9_4_5_effectiveDimension`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional consequence, with safe zero-diameter handling. |
...
| Chapter 9 | Remark 9.4.7 | Penalized unconstrained optimization gives a robust alternative to exact feasibility. | `HDP.Chapter9.remark_9_4_7_unconstrainedOptimization`; `exercise_9_20`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Corrected tuning/zero branches are proved conditionally on the appendix-proved lower principle. |
| Chapter 9 | Corollary 9.4.8 | Sparse vectors can be recovered from roughly `s log n` subgaussian measurements. | `HDP.Chapter9.theorem_9_4_8_sparseRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual `sqrt s` `l1` prior, but the recovery theorem depends on the appendix-proved lower principle. |
| Chapter 9 | Remark 9.4.10 | Truncating the `l1` ball improves `log n` to `log(en/s)`. | `HDP.Chapter9.remark_9_4_10_logImprovement`; `exercise_9_26`; `exercise_9_26_improvedSparseRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Convexification and width are core; the advertised improved recovery conclusion is conditional. |
| Chapter 9 | Corollary 9.4.11 | Low-rank matrices can be recovered from roughly `rd` Gaussian measurements via nuclear norm. | `HDP.Chapter9.theorem_9_4_11_lowRankRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual nuclear-norm prior, but conditional on the appendix-proved lower principle. |
| Chapter 9 | Theorem 9.5.1 | `l1` minimization exactly recovers every sparse vector with high probability. | `HDP.Chapter9.theorem_9_5_1_exactSparseRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Uniform actual descent-set theorem, but its escape input is conditional on the appendix-proved lower principle. |
...
| Chapter 9 | Theorem 9.6.3 | A Gaussian matrix obeys a general deviation inequality for sublinear functionals with linear growth. | `HDP.Chapter9.theorem_9_6_3_generalMatrixDeviation`; `theorem_9_6_3_generalMatrixDeviation_envelope`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional core wrapper. It correctly replaces the PDF's impossible global boundedness by Euclidean linear growth and fixes the output dimension. |
| Chapter 9 | Theorem 9.7.1 | Two-sided Chevet controls the deviation of support suprema under a Gaussian matrix. | `HDP.Chapter9.theorem_9_7_1_twoSidedChevet_set_ENN`; `theorem_9_7_1_twoSidedChevet_set`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional wrapper; it correctly uses `rad(S)` where the PDF explanatory clause says `rad(T)`. |
...
| Chapter 9 | Example 9.7.4 | A random proportional-dimensional image of the cube is almost round. | `HDP.Chapter9.example_9_7_4_cubeNearlyRound_set`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual cube body and image bridges are core; the almost-round probability conclusion is conditional. |
| Chapter 9 | Eq. (9.11) | Uniform matrix deviations satisfy a width-plus-radius high-probability bound. | `HDP.Chapter9.remark_9_1_4_matrixDeviation_highProbability_set`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual-set conditional wrapper; the lower-principle witness is appendix-proved. |
| Chapter 9 | Eq. (9.13) | Expected cross-polytope section diameter is `O(sqrt(log n/n))`. | `HDP.Chapter9.example_9_3_2_crossPolytopeBody_mStar`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional M-star consequence; Lean corrects the display's stray `T`. |
| Chapter 9 | Eq. (9.14) | Escape occurs when `m >= C K^4 w(T)^2`. | hypothesis/conclusion of `HDP.Chapter9.theorem_9_3_4_escape`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Conditional on the appendix-proved lower principle. |
...
| Chapter 9 | Eq. (9.19) | Expected constrained-recovery error is `O(K^2 w(T)/sqrt m)`. | `HDP.Chapter9.theorem_9_4_4_constrainedRecovery`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Actual-prior conditional result. |
| Chapter 9 | Eq. (9.45) | Gaussian image is sandwiched between inner and outer Euclidean balls. | `HDP.Chapter9.DvoretzkyMilmanSetConclusion`; `theorem_9_7_2_dvoretzkyMilman`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Predicate is core; the `0.99` theorem is conditional. Output balls correctly live in `R^m`. |
| Chapter 9 | Exercise 9.2 | Center matrix deviations around `E ‖Ax‖`. | `HDP.Chapter9.exercise_9_2_centeredMatrixDeviation_envelope`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Promoted proof is conditional because Theorem 9.1.1 is. |
| Chapter 9 | Exercise 9.3 | Convert norm deviations into quadratic norm deviations. | `HDP.Chapter9.exercise_9_3_quadraticMatrixDeviation_envelope`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Correct arbitrary-set envelope, conditional on the appendix-proved lower principle. |
| Chapter 9 | Exercise 9.9 | High-probability low-rank covariance estimation. | `HDP.Chapter9.exercise_9_9_lowRankCovariance_highProbability_direct`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Direct-row corrected theorem, but conditional in core and discharged by the appendix. |
...
| Chapter 9 | Exercise 9.19 | Constrained recovery in the noisy linear model. | `HDP.Chapter9.exercise_9_19`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Corrects the PDF's random noise norm left outside expectation and makes integrability explicit. |
| Chapter 9 | Exercise 9.20 | Penalized unconstrained recovery with a rigorously tuned parameter. | `HDP.Chapter9.exercise_9_20`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Fixes undefined tuning at `x=0`, attainment, and the meaning of asymptotic comparison. |
...
| Chapter 9 | Exercise 9.37 | General-norm Johnson--Lindenstrauss from general matrix deviation. | `HDP.Chapter9.exercise_9_37_generalNormJL`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | Precise quantitative theorem, conditional with Theorem 9.6.3. |
| Chapter 9 | Sec. 9.7 opening | General-norm JL follows from the general matrix-deviation theorem. | `HDP.Chapter9.exercise_9_37_generalNormJL`; appendix-proved lower-principle witness | `HighDimensionalProbability/Chapter9_DeviationsOfRandomMatricesOnSets.lean`; `HighDimensionalProbability/Appendix/MajorizingMeasureLower.lean` | PDF cites Theorem 9.1.1 here, but the intended source is Theorem 9.6.3; Lean fixes the citation and exposes its isolated appendix dependency. |
...
| Chapter 1 | MGF and moments | FORMALIZED | The moment-generating function is `E exp(lambda X)`. | The real Bochner helper represents only the finite/integrable regime; the book's MGF may be `+∞`. The prose derivative interpretation is not packaged separately. |
...
| Chapter 1 | (1.11) | FORMALIZED | `L^2` inner product is `E[XY]`. | `MeasureTheory.L2.inner_def` is exact for two `L²` variables; the unrestricted representative-level real-integral helper is totalized to zero off domain. |
| Chapter 1 | (1.12) | FORMALIZED | Standard deviation equals the `L^2` norm of the centered variable. | Exact. |
| Chapter 1 | (1.13), vector expectation/covariance | FORMALIZED | Covariance is the centered `L^2` pairing; vector means and covariance matrices are coordinatewise. | Exact on the measurable finite-second-moment domain through `covariance_eq_sub`, `expectation_vector_apply`, and `covarianceMatrix_eq_secondMoment_sub_mean`; raw real-integral covariance helpers are totalized off that domain. |
...
| Chapter 2 | Definition 2.6.4; (2.18) | FORMALIZED | Subgaussian variables and the `psi_2` infimum norm. | The raw predicate omits measurability and probability-space assumptions; coverage uses the restricted characterization/attainment family, which supplies them explicitly. |
...
| Chapter 2 | Definition 2.8.4; (2.25) | FORMALIZED | Subexponential variables and the `psi_1` infimum norm. | The raw predicate omits measurability and probability-space assumptions; the source-facing characterization family supplies them explicitly. |
...
| Chapter 3 | Definition 3.2.5; (3.9) | FORMALIZED | Isotropy means `E[XX^T]=I`, equivalently `E<X,v>^2=‖v‖^2`. | Correctly uncentered, but the raw predicate omits measurability, probability, and finite-second-moment assumptions and inherits Bochner-integral totalization. Coverage uses the restricted Chapter 3 moment identities. |
...
| Chapter 3 | Definition 3.4.1 | FORMALIZED | A vector is subgaussian if every one-dimensional marginal is; its vector `psi_2` norm is the supremum over unit directions. | The raw predicate inherits the scalar measurability/probability defect, and the real `sSup` is source-faithful only under its explicit boundedness premise. |
...
| Chapter 3 | Exercise 3.18 | PARTIAL | Ginibre/standard Gaussian matrix invariance. | `exercise_3_18` proves invariance of an abstract standard Gaussian under two already-supplied Hilbert isometries, but never constructs the Ginibre matrix law or proves that left/right multiplication by orthogonal matrices are those isometries. |
| Chapter 3 | Exercise 3.19 | PARTIAL | GOE representation and invariance. | `gaussianSymmetrizedLaw` is defined as `map S stdGaussian`, so `exercise_3_19a` is `rfl`; no symmetric-matrix space, Ginibre matrix, coordinate variances, or map `G ↦ (G+Gᵀ)/sqrt 2` is constructed. The target lacks `BorelSpace`, so a nonmeasurable pushforward can also collapse to the zero measure. Part (b) assumes the required commuting square. |
| Chapter 3 | Exercise 3.20 | PARTIAL | A Gaussian matrix sends orthogonal vectors to independent Gaussian images. | `exercise_3_20` proves only that the projections of an already-product Gaussian law are independent standard Gaussians; it never constructs `G,u,v` or identifies the law of `(Gu,Gv)` with that product. |
...
| Chapter 7 | Remark 7.2.1 | PARTIAL | For arbitrary index sets, expected suprema are interpreted as suprema of finite-subset expected maxima. | `extendedExpectedSupremum` has extended-real codomain, but each finite stage uses the real Bochner integral; Mathlib totalizes every nonintegrable finite maximum to `0`, losing the book's possible `+∞` value. |
...
| Chapter 8 | Eq. (8.37) | PARTIAL | Population squared prediction risk. | `populationRisk` is a real Bochner integral with no integrability premise, so a nonintegrable nonnegative squared loss is assigned the junk value `0` and can become a spurious minimizer. |
...
| Chapter 9 | Sec. 9.2.1, before Proposition 9.2.1 | FORMALIZED | Applying matrix deviation to the unit sphere recovers two-sided singular-value bounds. | This fixed-sphere corollary is proved directly and does not need the isolated appendix lower-majorizing-measure witness. |
